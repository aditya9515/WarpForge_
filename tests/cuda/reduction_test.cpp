...
std::vector<float> make_input(const std::size_t element_count, const bool negative_only) {
...
void run_case(const std::size_t element_count, const warpforge::ReductionOperation operation,
              const warpforge::ReductionVariant variant, const unsigned int block_size,
    ...
    CUDA_CHECK(cudaMemcpy(device_input.get(), input.data(), element_count * sizeof(float),
                          cudaMemcpyHostToDevice));

    warpforge::reduce_cuda(device_input.get(), element_count, operation, variant, workspace_a.get(),
                           workspace_b.get(), workspace_elements, device_output.get(), block_size);
    ...
    if (operation == warpforge::ReductionOperation::sum) {
        validation = warpforge::validate_reduction_sum(
            warpforge::reduction_sum_cpu(input.data(), input.size()), actual, input.size());
    } else {
    ...
    if (!validation.passed) {
        throw std::runtime_error(std::string(warpforge::reduction_operation_name(operation)) + "/" +
                                 ...
                                 std::to_string(element_count) + ", block " +
                                 std::to_string(block_size) + ", absolute error " +
...
void test_shapes_and_variants() {
    constexpr std::array<std::size_t, 16> sizes{
        1U,   2U,   17U,  31U,  32U,  33U,   127U,  255U,
        256U, 257U, 511U, 512U, 513U, 1003U, 4097U, 1U << 20U,
    };
    for (const std::size_t size : sizes) {
        for (const auto variant : variants) {
            run_case(size, warpforge::ReductionOperation::sum, variant,
                     warpforge::reduction_default_block_size);
            run_case(size, warpforge::ReductionOperation::maximum, variant,
                     warpforge::reduction_default_block_size);
        }
    }
}
...
void test_block_dispatch() {
    for (const unsigned int block_size : std::array<unsigned int, 5>{32U, 64U, 128U, 256U, 512U}) {
        run_case(1003U, warpforge::ReductionOperation::sum, warpforge::ReductionVariant::unrolled,
                 block_size);
        run_case(1003U, warpforge::ReductionOperation::maximum,
                 warpforge::ReductionVariant::warp_shuffle, block_size, true);
    }
}

void test_negative_only_maximum() {
    for (const auto variant : variants) {
        run_case(4097U, warpforge::ReductionOperation::maximum, variant,
                 warpforge::reduction_default_block_size, true);
    }
}
...
void test_invalid_arguments() {
    ...
    try {
        warpforge::reduce_cuda(nullptr, 0, warpforge::ReductionOperation::sum,
                               warpforge::ReductionVariant::warp_shuffle, nullptr, nullptr, 0,
                               nullptr);
    ...
    try {
        warpforge::reduce_cuda(input.get(), 1024U, warpforge::ReductionOperation::sum,
                               warpforge::ReductionVariant::naive_interleaved, nullptr, nullptr, 0,
                               output.get());
    } catch (const std::invalid_argument&) {
