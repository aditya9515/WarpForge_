...
void validate_backend(
    ...
    const auto observer = [&](
        ...
        if (!validation.passed) {
            throw std::runtime_error(
                ...
                std::to_string(validation.worst_index) + ", max absolute error " +
    ...
    stream.synchronize();
    expect(
        observed.size() == warpforge::miniinfer_intermediate_names().size(),
...
int main(const int argument_count, char** arguments) {
    try {
        ...
        expect(block.workspace().capacity_bytes() == workspace_capacity, "workspace capacity changed");
