...
void run_case(const std::size_t rows, const std::size_t columns, const float epsilon,
    ...
    warpforge::rmsnorm_cpu(input.data(), weight.data(), expected.data(), rows, columns, epsilon);
    ...
    for (const auto variant : variants) {
        warpforge::rmsnorm_cuda(device_input.get(), device_weight.get(), device_output.get(), rows,
                                columns, epsilon, variant);
        CUDA_CHECK(cudaDeviceSynchronize());
        if (count > 0U) {
            CUDA_CHECK(cudaMemcpy(actual.data(), device_output.get(), count * sizeof(float),
                                  cudaMemcpyDeviceToHost));
        }
        const auto validation = warpforge::validate_fp32(expected.data(), actual.data(), count,
                                                         warpforge::rmsnorm_tolerance(columns));
        if (!validation.passed) {
            throw std::runtime_error(std::string(warpforge::rmsnorm_variant_name(variant)) +
                                     " RMSNorm failed at index " +
...
void test_in_place() {
    ...
    warpforge::rmsnorm_cpu(input.data(), weight.data(), expected.data(), rows, columns, 1.0e-5F);
    ...
    CUDA_CHECK(cudaMemcpy(device_weight.get(), weight.data(), weight.size() * sizeof(float),
                          cudaMemcpyHostToDevice));
    warpforge::rmsnorm_cuda(values.get(), device_weight.get(), values.get(), rows, columns, 1.0e-5F,
                            warpforge::RmsNormVariant::block);
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaMemcpy(actual.data(), values.get(), actual.size() * sizeof(float),
                          cudaMemcpyDeviceToHost));
    if (!warpforge::validate_fp32(expected.data(), actual.data(), actual.size(),
                                  warpforge::rmsnorm_tolerance(columns))
             .passed) {
