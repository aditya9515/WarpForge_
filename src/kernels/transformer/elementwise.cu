...
void validate_binary(
    ...
    const std::size_t element_count) {
    if (element_count > 0U &&
        (left == nullptr || right == nullptr || output == nullptr)) {
        throw std::invalid_argument(
            "non-empty binary operation requires non-null inputs and output");
    }
}

[[nodiscard]] unsigned int grid_size(
    const std::size_t element_count,
...
void swiglu_unfused_cpu(
    ...
    if (element_count > 0U &&
        ...
        throw std::invalid_argument(
            "unfused SwiGLU intermediate must not alias inputs or output");
    }
...
void silu_cuda(
    ...
    silu_kernel<<<grid_size(element_count, block_size), block_size, 0U, stream>>>(
