...
void causal_mask_cpu(
    const float* input,
    float* output,
    const CausalMaskProblem& problem) {
    ...
    if (input == nullptr || output == nullptr) {
        throw std::invalid_argument(
            "non-empty causal mask requires non-null input and output");
    }
    const float negative_infinity = -std::numeric_limits<float>::infinity();
    for (std::size_t index = 0U; index < element_count; ++index) {
        const std::size_t key = index % problem.key_length;
        const std::size_t query =
            (index / problem.key_length) % problem.query_length;
        output[index] = key > problem.query_position_offset + query
            ? negative_infinity
            : input[index];
    }
}

void causal_mask_cuda(
    const float* input,
    float* output,
    const CausalMaskProblem& problem,
