...
void validate_residual_rmsnorm(const float* input, const float* residual, const float* weight,
                               const float* output, const std::size_t rows,
                               const std::size_t columns, const float epsilon) {
    const std::size_t elements = checked_elements(rows, columns);
    if (!std::isfinite(epsilon) || epsilon <= 0.0F) {
        throw std::invalid_argument("residual RMSNorm epsilon must be finite and positive");
    }
    if (elements > 0U &&
        (input == nullptr || residual == nullptr || weight == nullptr || output == nullptr)) {
        throw std::invalid_argument("non-empty residual RMSNorm requires non-null pointers");
    }
    if (elements > 0U && output == weight) {
        throw std::invalid_argument("residual RMSNorm output must not alias its weight vector");
    }
}

void validate_swiglu(const float* gate, const float* up, const float* output,
                     const std::size_t element_count) {
    if (element_count > 0U && (gate == nullptr || up == nullptr || output == nullptr)) {
        throw std::invalid_argument("non-empty fused SwiGLU requires non-null pointers");
    }
}

[[nodiscard]] unsigned int elementwise_grid_size(const std::size_t element_count,
...
__global__ void residual_rmsnorm_fused_kernel(const float* input, const float* residual,
...
__global__ void swiglu_fused_kernel(const float* gate, const float* up, float* output,
                                    const std::size_t element_count) {
...
std::size_t residual_rmsnorm_dynamic_shared_memory_bytes(const unsigned int block_size) {
    validate_block_size(block_size);
    return static_cast<std::size_t>(block_size) * sizeof(float);
}

void residual_rmsnorm_cpu(const float* input, const float* residual, const float* weight,
                          float* output, const std::size_t rows, const std::size_t columns,
                          const float epsilon) {
    validate_residual_rmsnorm(input, residual, weight, output, rows, columns, epsilon);
    ...
    for (std::size_t row = 0U; row < rows; ++row) {
        ...
        for (std::size_t column = 0U; column < columns; ++column) {
            ...
            output[offset + column] =
                static_cast<float>(value * inverse_rms * static_cast<double>(weight[column]));
        }
    }
}

void residual_rmsnorm_fused_cuda(const float* input, const float* residual, const float* weight,
                                 float* output, const std::size_t rows, const std::size_t columns,
                                 const float epsilon, const unsigned int block_size,
    ...
    residual_rmsnorm_fused_kernel<<<static_cast<unsigned int>(rows), block_size,
                                    residual_rmsnorm_dynamic_shared_memory_bytes(block_size),
                                    stream>>>(input, residual, weight, output, rows, columns,
                                              epsilon);
    CUDA_CHECK(cudaGetLastError());
}

void swiglu_fused_cpu(const float* gate, const float* up, float* output,
                      const std::size_t element_count) {
...
void swiglu_fused_cuda(const float* gate, const float* up, float* output,
    ...
    swiglu_fused_kernel<<<elementwise_grid_size(element_count, block_size), block_size, 0U,
                          stream>>>(gate, up, output, element_count);
    CUDA_CHECK(cudaGetLastError());
}
