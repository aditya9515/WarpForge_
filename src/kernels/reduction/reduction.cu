...
template <ReductionOperation Operation> __device__ __forceinline__ float identity_value();

template <> __device__ __forceinline__ float identity_value<ReductionOperation::sum>() {
    return 0.0F;
}

template <> __device__ __forceinline__ float identity_value<ReductionOperation::maximum>() {
...
template <>
__device__ __forceinline__ float combine_values<ReductionOperation::sum>(const float left,
                                                                         const float right) {
    return left + right;
}

template <>
__device__ __forceinline__ float combine_values<ReductionOperation::maximum>(const float left,
                                                                             const float right) {
    return fmaxf(left, right);
}

template <ReductionOperation Operation, unsigned int BlockSize>
__global__ void interleaved_kernel(const float* input, float* output,
                                   const std::size_t element_count) {
    extern __shared__ float shared[];
    const unsigned int thread = threadIdx.x;
    const std::size_t index = static_cast<std::size_t>(blockIdx.x) * BlockSize + thread;
    shared[thread] = index < element_count ? input[index] : identity_value<Operation>();
    __syncthreads();
    ...
    for (unsigned int stride = 1U; stride < BlockSize; stride <<= 1U) {
        ...
        if (shared_index < BlockSize && shared_index + stride < BlockSize) {
            shared[shared_index] =
                combine_values<Operation>(shared[shared_index], shared[shared_index + stride]);
        }
        __syncthreads();
    }
    if (thread == 0U) {
...
__global__ void shared_memory_kernel(const float* input, float* output,
    ...
    for (unsigned int stride = BlockSize / 2U; stride > 0U; stride >>= 1U) {
        if (thread < stride) {
            shared[thread] = combine_values<Operation>(shared[thread], shared[thread + stride]);
        }
        __syncthreads();
    }
    if (thread == 0U) {
...
template <ReductionOperation Operation, unsigned int BlockSize>
__global__ void reduced_divergence_kernel(const float* input, float* output,
                                          const std::size_t element_count) {
    extern __shared__ float shared[];
    const unsigned int thread = threadIdx.x;
    const std::size_t first = static_cast<std::size_t>(blockIdx.x) * (2U * BlockSize) + thread;
...
template <ReductionOperation Operation, unsigned int BlockSize>
__global__ void unrolled_kernel(const float* input, float* output,
                                const std::size_t element_count) {
    extern __shared__ float shared[];
    const unsigned int thread = threadIdx.x;
    const std::size_t first = static_cast<std::size_t>(blockIdx.x) * (2U * BlockSize) + thread;
...
template <ReductionOperation Operation, unsigned int BlockSize>
__global__ void warp_shuffle_kernel(const float* input, float* output,
                                    const std::size_t element_count) {
    ...
    for (int offset = 16; offset > 0; offset >>= 1) {
        value = combine_values<Operation>(value, __shfl_down_sync(full_mask, value, offset));
    }
    if (lane == 0U) {
...
template <ReductionOperation Operation, unsigned int BlockSize>
void launch_block(const ReductionVariant variant, const float* input, float* output,
                  const std::size_t element_count, const unsigned int grid_size,
                  cudaStream_t stream) {
...
template <ReductionOperation Operation>
void launch_pass(const ReductionVariant variant, const unsigned int block_size, const float* input,
                 float* output, const std::size_t element_count, const unsigned int grid_size,
                 cudaStream_t stream) {
    switch (block_size) {
    case 32U:
        launch_block<Operation, 32U>(variant, input, output, element_count, grid_size, stream);
        break;
    case 64U:
        launch_block<Operation, 64U>(variant, input, output, element_count, grid_size, stream);
        break;
    case 128U:
        launch_block<Operation, 128U>(variant, input, output, element_count, grid_size, stream);
        break;
    case 256U:
        launch_block<Operation, 256U>(variant, input, output, element_count, grid_size, stream);
        break;
    case 512U:
        launch_block<Operation, 512U>(variant, input, output, element_count, grid_size, stream);
        break;
    case 1024U:
        launch_block<Operation, 1024U>(variant, input, output, element_count, grid_size, stream);
        break;
    default:
...
std::size_t reduction_elements_per_block(const ReductionVariant variant,
...
std::size_t reduction_output_count(const std::size_t input_count, const ReductionVariant variant,
                                   const unsigned int block_size) {
    if (input_count == 0) {
        return 0;
    }
    const std::size_t elements_per_block = reduction_elements_per_block(variant, block_size);
    return 1U + (input_count - 1U) / elements_per_block;
}

std::size_t reduction_workspace_elements(const std::size_t input_count,
                                         const ReductionVariant variant,
                                         const unsigned int block_size) {
    const std::size_t first_output = reduction_output_count(input_count, variant, block_size);
    return first_output > 1U ? first_output : 0U;
}

std::size_t reduction_pass_count(const std::size_t input_count, const ReductionVariant variant,
                                 const unsigned int block_size) {
    if (input_count == 0) {
        return 0;
    }
...
std::size_t reduction_dynamic_shared_memory_bytes(const ReductionVariant variant,
                                                  const unsigned int block_size) {
    validate_block_size(block_size);
...
Tolerance reduction_sum_tolerance(const std::size_t element_count) {
    const double levels =
        element_count > 1U ? std::ceil(std::log2(static_cast<double>(element_count))) : 0.0;
    return {1.0e-5 * (1.0 + levels), 2.0e-5};
}

ValidationResult validate_reduction_sum(const double expected, const float actual,
    ...
    const double relative_error =
        expected == 0.0 ? (absolute_error == 0.0 ? 0.0 : std::numeric_limits<double>::infinity())
                        : absolute_error / std::abs(expected);
    const double allowed_error = tolerance.absolute + tolerance.relative * std::abs(expected);
...
void reduce_cuda(const float* input, const std::size_t element_count,
                 const ReductionOperation operation, const ReductionVariant variant,
                 float* workspace_a, float* workspace_b, const std::size_t workspace_capacity,
                 float* output, const unsigned int block_size, cudaStream_t stream) {
    validate_block_size(block_size);
    if (element_count == 0) {
    ...
    while (current_count > 1U) {
        const std::size_t output_count = reduction_output_count(current_count, variant, block_size);
        ...
        float* current_output =
            output_count == 1U ? output : (use_workspace_a ? workspace_a : workspace_b);
        const unsigned int grid_size = static_cast<unsigned int>(output_count);
        switch (operation) {
        case ReductionOperation::sum:
            launch_pass<ReductionOperation::sum>(variant, block_size, current_input, current_output,
                                                 current_count, grid_size, stream);
            break;
        case ReductionOperation::maximum:
            launch_pass<ReductionOperation::maximum>(variant, block_size, current_input,
                                                     current_output, current_count, grid_size,
                                                     stream);
            break;
        default:
