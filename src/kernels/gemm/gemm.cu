...
void validate_problem_dimensions(const GemmProblem& problem) {
    constexpr std::size_t cublas_limit =
...
__global__ void naive_fp32_kernel(
    const float* a,
    const float* b,
    float* c,
    const std::size_t m,
...
__device__ __forceinline__ float to_float(const Input value) {
    return static_cast<float>(value);
}

template <>
__device__ __forceinline__ float to_float<__half>(const __half value) {
    return __half2float(value);
}

template <typename Input>
__global__ void tiled_16_kernel(
    const Input* a,
    const Input* b,
    float* c,
    const std::size_t m,
    ...
    const std::size_t column =
        static_cast<std::size_t>(blockIdx.x) * tile_16 + local_column;
    float accumulator = 0.0F;
    ...
    for (std::size_t tile_begin = 0; tile_begin < k; tile_begin += tile_16) {
        ...
        for (unsigned int inner = 0; inner < tile_16; ++inner) {
            accumulator = fmaf(
                a_tile[local_row][inner], b_tile[inner][local_column], accumulator);
        }
        __syncthreads();
    }
    if (row < m && column < n) {
...
__global__ void coalesced_fp32_kernel(
    const float* a,
    const float* b,
    float* c,
    const std::size_t m,
    ...
    const std::size_t column =
        static_cast<std::size_t>(blockIdx.x) * tile_32 + local_column;
    ...
    for (std::size_t tile_begin = 0; tile_begin < k; tile_begin += tile_32) {
        ...
        for (unsigned int offset = 0; offset < rows_per_thread; ++offset) {
            ...
            a_tile[local_row][local_column] =
                row < m && inner < k ? a[row * k + inner] : 0.0F;
            const std::size_t b_row = tile_begin + local_row;
            b_tile[local_row][local_column] =
...
__global__ void register_blocked_fp32_kernel(
    const float* a,
    const float* b,
    float* c,
    const std::size_t m,
    ...
    for (std::size_t tile_begin = 0; tile_begin < k; tile_begin += register_k) {
        for (unsigned int index = linear_thread;
             index < register_tile * register_k;
            ...
            a_tile[local_row][local_inner] =
                row < m && inner < k ? a[row * k + inner] : 0.0F;
        }
        for (unsigned int index = linear_thread;
             index < register_k * register_tile;
             index += blockDim.x * blockDim.y) {
    ...
    for (unsigned int row_offset = 0; row_offset < thread_tile; ++row_offset) {
        ...
        for (unsigned int column_offset = 0; column_offset < thread_tile; ++column_offset) {
            const std::size_t column =
                block_column + threadIdx.x * thread_tile + column_offset;
            if (row < m && column < n) {
...
__global__ void wmma_fp16_fp32_kernel(
    const __half* a,
    const __half* b,
    float* c,
    const int m,
    ...
    wmma::store_matrix_sync(
        c + tile_row * n + tile_column, accumulator, n, wmma::mem_row_major);
#else
    static_cast<void>(a);
    static_cast<void>(b);
    static_cast<void>(c);
...
void launch_custom_fp32(
    ...
    const auto grid_16 = dim3{
        static_cast<unsigned int>((problem.n + tile_16 - 1U) / tile_16),
        static_cast<unsigned int>((problem.m + tile_16 - 1U) / tile_16),
        1U};
    switch (variant) {
        case GemmVariant::naive_fp32:
            naive_fp32_kernel<<<grid_16, dim3{tile_16, tile_16, 1U}, 0, stream>>>(
        ...
        case GemmVariant::coalesced_fp32: {
            const dim3 grid{
                static_cast<unsigned int>((problem.n + tile_32 - 1U) / tile_32),
                static_cast<unsigned int>((problem.m + tile_32 - 1U) / tile_32),
                1U};
            coalesced_fp32_kernel<<<grid, dim3{tile_32, 8U, 1U}, 0, stream>>>(
                a, b, c, problem.m, problem.n, problem.k);
            break;
        }
        case GemmVariant::register_blocked_fp32: {
            const dim3 grid{
                static_cast<unsigned int>((problem.n + register_tile - 1U) / register_tile),
                static_cast<unsigned int>((problem.m + register_tile - 1U) / register_tile),
                1U};
            register_blocked_fp32_kernel<<<grid, dim3{16U, 16U, 1U}, 0, stream>>>(
                a, b, c, problem.m, problem.n, problem.k);
            break;
        }
        default:
...
bool gemm_variant_supports_problem(
    const GemmVariant variant,
    const GemmProblem& problem) noexcept {
    if (variant != GemmVariant::wmma_fp16_fp32) {
        return true;
    }
    return problem.m % 16U == 0U && problem.n % 16U == 0U &&
           problem.k % 16U == 0U;
}
...
Tolerance gemm_fp32_tolerance(const std::size_t inner_dimension) noexcept {
    const double scale = std::max(1.0, std::ceil(std::log2(static_cast<double>(std::max<std::size_t>(1U, inner_dimension)))));
