...
void upload_tensor(
    ...
    destination.copy_from_host_async(
        source.data(), source.size() * sizeof(float), stream);
}
...
[[nodiscard]] std::size_t workspace_capacity(const MiniInferConfig& config) {
    ...
    elements = checked_sum(
        elements,
        checked_product(intermediate, 3U, "MiniInfer MLP workspace"),
        "MiniInfer workspace elements");
    elements = checked_sum(
        elements,
        checked_product(scores, 3U, "MiniInfer attention workspace"),
        "MiniInfer workspace elements");
    const std::size_t data_bytes = checked_product(elements, sizeof(float), "MiniInfer workspace bytes");
    const std::size_t alignment_slack = checked_product(
        activation_count, workspace_alignment - 1U, "MiniInfer workspace alignment");
...
[[nodiscard]] TensorShape qkv_shape(const MiniInferConfig& config) {
    return TensorShape{
        config.batch, config.sequence, config.attention_heads, config.head_dimension};
}

[[nodiscard]] TensorShape score_shape(const MiniInferConfig& config) {
    return TensorShape{
        config.batch, config.attention_heads, config.sequence, config.sequence};
}
...
void validate_miniinfer_config(const MiniInferConfig& config) {
    ...
    if (config.position_offset >
        std::numeric_limits<std::size_t>::max() - (config.sequence - 1U)) {
...
std::size_t miniinfer_hidden_element_count(const MiniInferConfig& config) {
    return checked_product(
        miniinfer_token_count(config), config.hidden_size, "MiniInfer hidden elements");
}

std::size_t miniinfer_intermediate_element_count(const MiniInferConfig& config) {
    return checked_product(
        miniinfer_token_count(config),
        config.intermediate_size,
        "MiniInfer intermediate elements");
}

std::size_t miniinfer_score_element_count(const MiniInferConfig& config) {
    const std::size_t head_groups = checked_product(
        config.batch, config.attention_heads, "MiniInfer attention head groups");
    const std::size_t rows = checked_product(
        head_groups, config.sequence, "MiniInfer attention rows");
...
MiniInferWeights::MiniInferWeights(const MiniInferConfig& config)
    : config_(checked_config(config)),
      ...
      attention_norm_view_(attention_norm_.view()),
      query_view_(query_.view()),
      key_view_(key_.view()),
      value_view_(value_.view()),
      output_view_(output_.view()),
      ffn_norm_view_(ffn_norm_.view()),
      gate_view_(gate_.view()),
      up_view_(up_.view()),
      down_view_(down_.view()) {}

void MiniInferWeights::upload(
    const MiniInferHostWeights& weights,
    const cudaStream_t stream) {
    const std::size_t hidden_square = checked_product(
        config_.hidden_size, config_.hidden_size, "MiniInfer hidden weight");
    const std::size_t hidden_intermediate = checked_product(
        config_.hidden_size, config_.intermediate_size, "MiniInfer MLP weight");
    validate_host_vector(weights.attention_norm_weight, config_.hidden_size, "attention_norm_weight");
...
std::size_t MiniInferWeights::parameter_bytes() const noexcept {
    return attention_norm_.byte_size() + query_.byte_size() + key_.byte_size() +
           value_.byte_size() + output_.byte_size() + ffn_norm_.byte_size() +
           gate_.byte_size() + up_.byte_size() + down_.byte_size();
}
...
void MiniInferBlock::upload_weights(
    const MiniInferHostWeights& weights,
    const cudaStream_t stream) {
    weights_.upload(weights, stream);
}

void MiniInferBlock::forward(
    ...
    const GemmDispatch dispatch{
        backend,
        backend == GemmBackend::custom ? GemmVariant::register_blocked_fp32
    ...
    rmsnorm_cuda(
        input.data_as<float>(),
        attention_norm_weight.data_as<float>(),
        activation.input_norm.data_as<float>(),
        tokens,
        config_.hidden_size,
        config_.rms_norm_epsilon,
        RmsNormVariant::block,
        rmsnorm_default_block_size,
        stream);
    ...
    const RopeProblem rope_problem{
        config_.batch,
        config_.sequence,
        ...
        config_.rope_base};
    rope_cuda(
        activation.query.data_as<float>(),
        activation.query_rope.data_as<float>(),
        rope_problem,
        rope_default_block_size,
        stream);
    observe("query_rope", activation.query_rope);
    rope_cuda(
        activation.key.data_as<float>(),
    ...
    const AttentionProblem attention_problem{
        config_.batch,
        config_.sequence,
        config_.attention_heads,
        config_.head_dimension};
    attention_scores_cuda(
        activation.query_rope.data_as<float>(),
        activation.key_rope.data_as<float>(),
        activation.attention_scores.data_as<float>(),
        attention_problem,
        attention_default_block_size,
        stream);
    observe("attention_scores", activation.attention_scores);
    causal_mask_cuda(
        activation.attention_scores.data_as<float>(),
        activation.masked_scores.data_as<float>(),
        {config_.batch,
         config_.attention_heads,
         config_.sequence,
         config_.sequence,
         0U},
        causal_mask_default_block_size,
        stream);
    observe("masked_scores", activation.masked_scores);
    softmax_cuda(
        activation.masked_scores.data_as<float>(),
        activation.attention_probabilities.data_as<float>(),
        config_.batch * config_.attention_heads * config_.sequence,
        config_.sequence,
        SoftmaxVariant::block,
        softmax_default_block_size,
        stream);
    observe("attention_probabilities", activation.attention_probabilities);
    attention_value_cuda(
        activation.attention_probabilities.data_as<float>(),
        activation.value.data_as<float>(),
        activation.attention_context.data_as<float>(),
        attention_problem,
        attention_default_block_size,
        stream);
    ...
    gemm_cuda(
        activation.attention_context,
        output_weight,
        activation.attention_output,
        projection,
        dispatch,
        cublas_handle,
        stream);
    ...
    add_cuda(
        input.data_as<float>(),
        activation.attention_output.data_as<float>(),
        activation.attention_residual.data_as<float>(),
        hidden_elements,
        elementwise_default_block_size,
        stream);
    ...
    rmsnorm_cuda(
        activation.attention_residual.data_as<float>(),
        ffn_norm_weight.data_as<float>(),
        activation.post_attention_norm.data_as<float>(),
        tokens,
        config_.hidden_size,
        config_.rms_norm_epsilon,
        RmsNormVariant::block,
    ...
    gemm_cuda(
        activation.post_attention_norm,
        gate_weight,
        activation.mlp_gate,
        expand,
        dispatch,
    ...
    swiglu_fused_cuda(
        activation.mlp_gate.data_as<float>(),
        activation.mlp_up.data_as<float>(),
        activation.mlp_swiglu.data_as<float>(),
    ...
    gemm_cuda(
        activation.mlp_swiglu,
        down_weight,
        activation.mlp_output,
        contract,
        dispatch,
        cublas_handle,
        stream);
    ...
    add_cuda(
        activation.attention_residual.data_as<float>(),
        activation.mlp_output.data_as<float>(),
...
Tolerance miniinfer_intermediate_tolerance(
    ...
    const double accumulation_scale = std::max(
        1.0,
