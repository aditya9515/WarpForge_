...
template <typename T> class LocalPinnedBuffer final {
  public:
...
void validate_options(const Options& options) {
    ...
    if (options.rows > std::numeric_limits<std::size_t>::max() / options.columns ||
        options.rows * options.columns > std::numeric_limits<std::size_t>::max() / sizeof(float)) {
        throw std::out_of_range("matrix dimensions exceed the addressable FP32 range");
    }
}

std::vector<double> measure_host_operation(const warpforge::BenchmarkConfig& config,
...
warpforge::BenchmarkResult make_result(const Options& options, std::string operation,
    ...
    result.metadata = warpforge::make_benchmark_metadata(
        std::move(operation), std::move(implementation), "fp32", std::move(dimensions), launch);
...
void record_result(const Options& options, const std::string& filename,
                   warpforge::BenchmarkResult result, std::vector<RecordedResult>& records) {
    if (!result.validation.passed) {
        throw std::runtime_error(filename + " failed validation");
    }
    warpforge::write_benchmark_json(result, options.output_directory / (filename + ".json"));
    records.push_back({filename, std::move(result)});
}

void write_summary(const Options& options, const std::vector<RecordedResult>& records) {
    ...
    for (const auto& record : records) {
        ...
        output << result.schema_version << ',' << record.filename << ','
               ...
               << (result.validation.passed ? "true" : "false") << ',' << result.metadata.git_commit
               << '\n';
    }
}

void fill_random(float* output, const std::size_t element_count, std::mt19937& generator) {
...
void run_saxpy_sweep(const Options& options, std::vector<RecordedResult>& records) {
    ...
    for (const unsigned int block_size : block_sizes) {
        ...
        warpforge::saxpy_cuda(saxpy_alpha, device_input.get(), device_output.get(),
                              options.element_count, block_size);
        CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaMemcpy(actual.data(), device_output.get(), bytes, cudaMemcpyDeviceToHost));
        const auto validation = warpforge::validate_fp32(expected.data(), actual.data(),
                                                         actual.size(), {1.0e-5, 1.0e-5});

        CUDA_CHECK(cudaMemcpy(device_output.get(), initial.data(), bytes, cudaMemcpyHostToDevice));
        auto samples =
            warpforge::measure_cuda_kernel(options.benchmark, nullptr, [&](cudaStream_t stream) {
                warpforge::saxpy_cuda(saxpy_alpha, device_input.get(), device_output.get(),
                                      options.element_count, block_size, stream);
            });
        ...
        auto result = make_result(options, "saxpy", "cuda_block_" + std::to_string(block_size),
                                  {{"elements", static_cast<std::uint64_t>(options.element_count)}},
                                  launch, validation, std::move(samples), "kernel-only");
        result.metrics["effective_bandwidth_gbps_from_median"] =
            bandwidth_gbps(static_cast<double>(bytes) * 3.0, result.statistics.median_ms);
        record_result(options, "saxpy_block_" + std::to_string(block_size), std::move(result),
                      records);
    }
}
...
void run_copy_sweep(const Options& options, std::vector<RecordedResult>& records) {
    ...
    for (const unsigned int block_size : block_sizes) {
        warpforge::memory_copy_cuda(device_input.get(), device_output.get(), options.element_count,
                                    block_size);
        CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaMemcpy(actual.data(), device_output.get(), bytes, cudaMemcpyDeviceToHost));
        const auto validation =
            warpforge::validate_fp32(input.data(), actual.data(), actual.size(), {0.0, 0.0});

        auto samples =
            warpforge::measure_cuda_kernel(options.benchmark, nullptr, [&](cudaStream_t stream) {
                warpforge::memory_copy_cuda(device_input.get(), device_output.get(),
                                            options.element_count, block_size, stream);
            });
        warpforge::LaunchConfiguration launch;
        launch.grid = {warpforge::memory_grid_size(options.element_count, block_size), 1U, 1U};
        launch.block = {block_size, 1U, 1U};
        auto result =
            make_result(options, "memory_copy", "cuda_block_" + std::to_string(block_size),
                        {{"elements", static_cast<std::uint64_t>(options.element_count)}}, launch,
                        validation, std::move(samples), "kernel-only");
        result.metrics["effective_bandwidth_gbps_from_median"] =
            bandwidth_gbps(static_cast<double>(bytes) * 2.0, result.statistics.median_ms);
        record_result(options, "copy_block_" + std::to_string(block_size), std::move(result),
                      records);
    }
}

void run_stride_sweep(const Options& options, std::vector<RecordedResult>& records) {
    const std::size_t logical_elements = std::min(options.element_count, maximum_stride_elements);
    ...
    CUDA_CHECK(cudaMemcpy(device_input.get(), input.data(), source_elements * sizeof(float),
                          cudaMemcpyHostToDevice));

    for (const std::size_t stride : strides) {
        warpforge::strided_copy_cpu(input.data(), expected.data(), logical_elements, stride);
        warpforge::strided_copy_cuda(device_input.get(), device_output.get(), logical_elements,
                                     stride);
        CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaMemcpy(actual.data(), device_output.get(), logical_elements * sizeof(float),
                              cudaMemcpyDeviceToHost));
        const auto validation =
            warpforge::validate_fp32(expected.data(), actual.data(), logical_elements, {0.0, 0.0});

        auto samples =
            warpforge::measure_cuda_kernel(options.benchmark, nullptr, [&](cudaStream_t stream) {
                warpforge::strided_copy_cuda(device_input.get(), device_output.get(),
                                             logical_elements, stride,
                                             warpforge::memory_default_block_size, stream);
            });
        warpforge::LaunchConfiguration launch;
        launch.grid = {warpforge::memory_grid_size(logical_elements), 1U, 1U};
        launch.block = {warpforge::memory_default_block_size, 1U, 1U};
        auto result =
            make_result(options, "strided_copy", "stride_" + std::to_string(stride),
                        ...
                        launch, validation, std::move(samples), "kernel-only");
        result.metrics["effective_bandwidth_gbps_from_median"] =
            bandwidth_gbps(static_cast<double>(logical_elements * sizeof(float)) * 2.0,
                           result.statistics.median_ms);
        record_result(options, "strided_stride_" + std::to_string(stride), std::move(result),
                      records);
    }
}
...
void run_transpose(const Options& options, std::vector<RecordedResult>& records) {
    ...
    std::mt19937 generator(static_cast<std::uint32_t>(options.benchmark.seed));
    fill_random(input.data(), input.size(), generator);
    warpforge::transpose_cpu(input.data(), expected.data(), options.rows, options.columns);
    ...
    for (const bool tiled : std::array<bool, 2>{false, true}) {
        if (tiled) {
            warpforge::transpose_tiled_cuda(device_input.get(), device_output.get(), options.rows,
                                            options.columns);
        } else {
            warpforge::transpose_naive_cuda(device_input.get(), device_output.get(), options.rows,
                                            options.columns, naive_block);
        }
        CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaMemcpy(actual.data(), device_output.get(), bytes, cudaMemcpyDeviceToHost));
        const auto validation =
            warpforge::validate_fp32(expected.data(), actual.data(), element_count, {0.0, 0.0});

        auto samples =
            warpforge::measure_cuda_kernel(options.benchmark, nullptr, [&](cudaStream_t stream) {
                if (tiled) {
                    warpforge::transpose_tiled_cuda(device_input.get(), device_output.get(),
                                                    options.rows, options.columns, stream);
                } else {
                    warpforge::transpose_naive_cuda(device_input.get(), device_output.get(),
                                                    options.rows, options.columns, naive_block,
                                                    stream);
                }
            });
        ...
        if (tiled) {
            ...
            launch.block = {warpforge::transpose_tile_dimension, warpforge::transpose_block_rows,
                            1U};
        } else {
        ...
        auto result = make_result(options, "matrix_transpose", implementation,
        ...
        if (tiled) {
            result.metrics["static_shared_memory_bytes"] =
                ...
                static_cast<double>(warpforge::transpose_tile_dimension + 1U) * sizeof(float);
        }
...
void run_transfer_variant(const Options& options, const std::string& implementation,
                          float* expected, float* actual, const std::size_t element_count,
                          Work&& work, std::vector<RecordedResult>& records) {
    work();
    const auto validation = warpforge::validate_fp32(expected, actual, element_count, {0.0, 0.0});
    auto samples = measure_host_operation(options.benchmark, work);
    const warpforge::LaunchConfiguration launch{};
    auto result = make_result(options, "transfer_roundtrip", implementation,
                              {{"elements", static_cast<std::uint64_t>(element_count)}}, launch,
                              validation, std::move(samples), "end-to-end");
    result.metrics["effective_bandwidth_gbps_from_median"] = bandwidth_gbps(
        static_cast<double>(element_count * sizeof(float)) * 2.0, result.statistics.median_ms);
...
void run_transfer_comparisons(const Options& options, std::vector<RecordedResult>& records) {
    ...
    run_transfer_variant(
        options, "pageable_sync", pageable_input.data(), pageable_output.data(),
        options.element_count,
        [&]() {
            CUDA_CHECK(
                cudaMemcpy(device.get(), pageable_input.data(), bytes, cudaMemcpyHostToDevice));
            CUDA_CHECK(
                cudaMemcpy(pageable_output.data(), device.get(), bytes, cudaMemcpyDeviceToHost));
        },
        records);
    run_transfer_variant(
        options, "pinned_sync", pinned_input.get(), pinned_output.get(), options.element_count,
        [&]() {
            CUDA_CHECK(cudaMemcpy(device.get(), pinned_input.get(), bytes, cudaMemcpyHostToDevice));
            CUDA_CHECK(
                cudaMemcpy(pinned_output.get(), device.get(), bytes, cudaMemcpyDeviceToHost));
        },
        records);
    run_transfer_variant(
        options, "pageable_async", pageable_input.data(), pageable_output.data(),
        options.element_count,
        [&]() {
            CUDA_CHECK(cudaMemcpyAsync(device.get(), pageable_input.data(), bytes,
                                       cudaMemcpyHostToDevice, stream.get()));
            CUDA_CHECK(cudaMemcpyAsync(pageable_output.data(), device.get(), bytes,
                                       cudaMemcpyDeviceToHost, stream.get()));
            CUDA_CHECK(cudaStreamSynchronize(stream.get()));
        },
        records);
    run_transfer_variant(
        options, "pinned_async", pinned_input.get(), pinned_output.get(), options.element_count,
        [&]() {
            CUDA_CHECK(cudaMemcpyAsync(device.get(), pinned_input.get(), bytes,
                                       cudaMemcpyHostToDevice, stream.get()));
            CUDA_CHECK(cudaMemcpyAsync(pinned_output.get(), device.get(), bytes,
                                       cudaMemcpyDeviceToHost, stream.get()));
            CUDA_CHECK(cudaStreamSynchronize(stream.get()));
        },
        records);
...
void run_pipeline_variant(const Options& options, const bool two_streams,
                          std::vector<RecordedResult>& records) {
    const std::size_t chunk_count = std::min(pipeline_chunk_count, options.element_count);
    const std::size_t chunk_capacity = (options.element_count + chunk_count - 1U) / chunk_count;
    ...
    warpforge::saxpy_cpu(saxpy_alpha, input.get(), expected.data(), expected.size());

    std::array<LocalStream, 2> streams{};
    std::array<LocalDeviceBuffer<float>, 2> device_input{LocalDeviceBuffer<float>{chunk_capacity},
                                                         LocalDeviceBuffer<float>{chunk_capacity}};
    std::array<LocalDeviceBuffer<float>, 2> device_output{LocalDeviceBuffer<float>{chunk_capacity},
                                                          LocalDeviceBuffer<float>{chunk_capacity}};
    ...
    const auto execute = [&]() {
        for (std::size_t chunk = 0; chunk < chunk_count; ++chunk) {
            ...
            const std::size_t count = std::min(chunk_capacity, options.element_count - offset);
            const std::size_t bytes = count * sizeof(float);
            const cudaStream_t stream = streams[stream_index].get();
            CUDA_CHECK(cudaMemcpyAsync(device_input[stream_index].get(), input.get() + offset,
                                       bytes, cudaMemcpyHostToDevice, stream));
            CUDA_CHECK(cudaMemcpyAsync(device_output[stream_index].get(), initial.get() + offset,
                                       bytes, cudaMemcpyHostToDevice, stream));
            warpforge::saxpy_cuda(saxpy_alpha, device_input[stream_index].get(),
                                  device_output[stream_index].get(), count,
                                  warpforge::memory_default_block_size, stream);
            CUDA_CHECK(cudaMemcpyAsync(output.get() + offset, device_output[stream_index].get(),
    ...
    execute();
    const auto validation =
        warpforge::validate_fp32(expected.data(), output.get(), expected.size(), {1.0e-5, 1.0e-5});
    ...
    const warpforge::LaunchConfiguration launch{};
    auto result = make_result(options, "saxpy_pipeline", implementation,
                              {{"chunks", static_cast<std::uint64_t>(chunk_count)},
                               {"elements", static_cast<std::uint64_t>(options.element_count)},
                               {"streams", two_streams ? 2U : 1U}},
                              launch, validation, std::move(samples), "end-to-end");
    result.metrics["effective_bandwidth_gbps_from_median"] =
        bandwidth_gbps(static_cast<double>(options.element_count * sizeof(float)) * 3.0,
                       result.statistics.median_ms);
...
void print_results(const std::vector<RecordedResult>& records) {
    std::cout << std::left << std::setw(34) << "Experiment" << std::right << std::setw(14)
              << "Median ms" << std::setw(14) << "P95 ms" << std::setw(16) << "Logical GB/s"
              << '\n';
    std::cout << std::string(78, '-') << '\n';
    std::cout << std::fixed << std::setprecision(4);
    for (const auto& record : records) {
        const auto bandwidth = record.result.metrics.find("effective_bandwidth_gbps_from_median");
        std::cout << std::left << std::setw(34) << record.filename << std::right << std::setw(14)
                  << record.result.statistics.median_ms << std::setw(14)
                  << record.result.statistics.p95_ms << std::setw(16)
