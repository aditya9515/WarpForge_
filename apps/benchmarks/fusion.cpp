...
void validate_options(const Options& options) {
    ...
    constexpr std::size_t max_fp32_count = std::numeric_limits<std::size_t>::max() / sizeof(float);
    if (options.rows * options.columns > max_fp32_count || options.element_count > max_fp32_count ||
...
unsigned int checked_grid_size(const std::size_t element_count, const unsigned int block_size) {
...
RecordedResult measure_cuda_case(const std::string& filename, warpforge::BenchmarkMetadata metadata,
                                 const warpforge::BenchmarkConfig& config,
                                 const warpforge::Tolerance tolerance,
                                 const std::vector<float>& expected, float* device_output,
                                 const std::size_t logical_reads, const std::size_t logical_writes,
                                 const warpforge::CudaWork& work, const cudaStream_t stream) {
    work(stream);
...
void benchmark_residual_rmsnorm(const Options& options, std::vector<RecordedResult>& records) {
    ...
    CUDA_CHECK(cudaMemcpy(device_residual.get(), residual.data(), count * sizeof(float),
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(device_weight.get(), weight.data(), weight.size() * sizeof(float),
                          cudaMemcpyHostToDevice));
    ...
    auto separate_metadata = warpforge::make_benchmark_metadata(
        "residual_rmsnorm", "separate", "fp32",
        {{"columns", options.columns}, {"kernel_launches", 2U}, {"rows", options.rows}}, launch);
    records.push_back(measure_cuda_case(
        "residual_rmsnorm_separate.json", std::move(separate_metadata), options.benchmark,
        warpforge::rmsnorm_tolerance(options.columns), expected, device_output.get(), 4U, 2U,
        [&](const cudaStream_t launch_stream) {
            warpforge::add_cuda(device_input.get(), device_residual.get(),
                                device_intermediate.get(), count,
                                warpforge::fusion_default_block_size, launch_stream);
            warpforge::rmsnorm_cuda(device_intermediate.get(), device_weight.get(),
                                    device_output.get(), options.rows, options.columns, epsilon,
    ...
    auto fused_metadata = warpforge::make_benchmark_metadata(
        "residual_rmsnorm", "fused", "fp32",
        {{"columns", options.columns}, {"kernel_launches", 1U}, {"rows", options.rows}}, launch);
    records.push_back(measure_cuda_case(
        "residual_rmsnorm_fused.json", std::move(fused_metadata), options.benchmark,
        warpforge::rmsnorm_tolerance(options.columns), expected, device_output.get(), 3U, 1U,
        [&](const cudaStream_t launch_stream) {
            warpforge::residual_rmsnorm_fused_cuda(
                device_input.get(), device_residual.get(), device_weight.get(), device_output.get(),
                options.rows, options.columns, epsilon, warpforge::fusion_default_block_size,
                launch_stream);
        },
        stream.get()));
}

void benchmark_swiglu(const Options& options, std::vector<RecordedResult>& records) {
    ...
    LocalStream stream;
    CUDA_CHECK(
        cudaMemcpy(device_gate.get(), gate.data(), count * sizeof(float), cudaMemcpyHostToDevice));
    CUDA_CHECK(
    ...
    auto separate_metadata = warpforge::make_benchmark_metadata(
        "swiglu", "separate", "fp32", {{"elements", count}, {"kernel_launches", 2U}}, launch);
    records.push_back(measure_cuda_case(
        "swiglu_separate.json", std::move(separate_metadata), options.benchmark, {1.0e-5, 1.0e-5},
        expected, device_output.get(), 3U, 2U,
        [&](const cudaStream_t launch_stream) {
    ...
    auto fused_metadata = warpforge::make_benchmark_metadata(
        "swiglu", "fused", "fp32", {{"elements", count}, {"kernel_launches", 1U}}, launch);
    records.push_back(measure_cuda_case(
        "swiglu_fused.json", std::move(fused_metadata), options.benchmark, {1.0e-5, 1.0e-5},
        expected, device_output.get(), 2U, 1U,
        [&](const cudaStream_t launch_stream) {
            warpforge::swiglu_fused_cuda(device_gate.get(), device_up.get(), device_output.get(),
                                         count, warpforge::fusion_default_block_size,
                                         launch_stream);
        },
        stream.get()));
}

void benchmark_pipeline_variant(const Options& options, const bool two_streams,
    ...
    std::array<warpforge::CudaEvent, 2> completion_events{
        warpforge::CudaEvent{cudaEventDisableTiming}, warpforge::CudaEvent{cudaEventDisableTiming}};
    std::array<LocalDeviceBuffer<float>, 2> buffers{LocalDeviceBuffer<float>{chunk_capacity},
    ...
    const auto execute = [&]() {
        for (std::size_t chunk = 0U; chunk < chunks; ++chunk) {
            ...
            warpforge::silu_cuda(buffers[stream_index].get(), buffers[stream_index].get(),
                                 chunk_elements, warpforge::fusion_default_block_size, stream);
    ...
    execute();
    const auto validation =
        warpforge::validate_fp32(expected.data(), output.get(), count, {1.0e-5, 1.0e-5});
    if (!validation.passed) {
        throw std::runtime_error(std::string(two_streams ? "two" : "single") +
    ...
    auto metadata = warpforge::make_benchmark_metadata(
        "pinned_silu_pipeline", implementation, "fp32",
        {{"chunks", chunks}, {"elements", count}, {"streams", active_streams}},
        {{checked_grid_size(chunk_capacity, warpforge::fusion_default_block_size), 1U, 1U},
         {warpforge::fusion_default_block_size, 1U, 1U},
         0U});
    ...
    warpforge::BenchmarkResult result{1,
                                      ...
                                      std::move(samples)};
    records.push_back({"pipeline_" + implementation + ".json", std::move(result)});
}
...
void write_results(const std::filesystem::path& directory,
    ...
    for (const auto& record : records) {
        ...
        summary << result.schema_version << ',' << record.filename << ','
...
void print_results(const std::vector<RecordedResult>& records) {
    for (const auto& record : records) {
        std::cout << record.result.metadata.operation << '/'
                  ...
                  << record.result.metrics.at("speedup_vs_operation_baseline") << "x baseline\n";
    }
}
