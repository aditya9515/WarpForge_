...
void print_usage() {
    std::cout << "Usage: warpforge-miniinfer [options]\n"
...
[[nodiscard]] std::vector<warpforge::GemmBackend>
selected_backends(const BackendSelection selection) {
...
void merge_validation(warpforge::ValidationResult& aggregate,
                      const warpforge::ValidationResult& current, const std::size_t index_offset) {
    ...
    aggregate.mean_absolute_error =
        aggregate.element_count == 0U
            ? 0.0
            : (prior_error_sum + current_error_sum) / static_cast<double>(aggregate.element_count);
...
BackendValidation validate_backend(warpforge::MiniInferBlock& block,
    ...
    const auto observer = [&](const std::string_view name, const warpforge::TensorView& view,
        ...
        const warpforge::ValidationResult current = warpforge::validate_fp32(
            expected->second.data(), actual.data(), actual.size(), tolerance);
        if (!current.passed) {
            throw std::runtime_error(std::string(warpforge::gemm_backend_name(backend)) +
                                     " backend " + std::string(name) + " failed at index " +
    ...
    block.forward(input, output, backend, cublas_handle, stream.native_handle(), observer);
    stream.synchronize();
...
std::vector<double>
measure_end_to_end(const warpforge::BenchmarkConfig& config, warpforge::MiniInferBlock& block,
                   const warpforge::MiniInferFixture& fixture, warpforge::Tensor& input,
                   warpforge::Tensor& output, warpforge::TensorView& input_view,
                   warpforge::TensorView& output_view, const warpforge::GemmBackend backend,
                   const cublasHandle_t cublas_handle, warpforge::CudaStream& stream) {
    ...
    const auto run_once = [&] {
        ...
        block.forward(input_view, output_view, backend, cublas_handle, stream.native_handle());
        output.copy_to_host_async(host_output.data(), host_output.size() * sizeof(float),
                                  stream.native_handle());
        stream.synchronize();
    };
...
warpforge::BenchmarkResult make_result(const Options& options,
                                       const warpforge::MiniInferFixture& fixture,
                                       const warpforge::MiniInferBlock& block,
                                       const warpforge::GemmBackend backend,
                                       std::string timing_scope, std::vector<double> samples,
    ...
    auto metadata = warpforge::make_benchmark_metadata(
        "miniinfer_block", implementation, "fp32",
...
void write_results(const std::filesystem::path& directory,
    ...
    for (const auto& record : records) {
        ...
        summary << result.schema_version << ','
                ...
                << result.metadata.implementation << ',' << result.metadata.timing_scope << ','
    ...
    std::ofstream validation_output(directory / "validation.csv",
    ...
    for (const auto& record : validations) {
        validation_output << record.backend << ',' << record.name << ','
                          << record.validation.element_count << ',' << record.tolerance.absolute
    ...
    try {
        ...
        for (const warpforge::GemmBackend backend : selected_backends(options.backend)) {
            ...
            BackendValidation validation =
                validate_backend(block, fixture, input_view, output_view, backend, handle, stream);
            validations.insert(validations.end(), validation.intermediates.begin(),
                               validation.intermediates.end());
            const auto launch = [&](const cudaStream_t launch_stream) {
                block.forward(input_view, output_view, backend, handle, launch_stream);
            };
            auto kernel_samples =
                warpforge::measure_cuda_kernel(options.benchmark, stream.native_handle(), launch);
            auto end_to_end_samples =
                measure_end_to_end(options.benchmark, block, fixture, input, output, input_view,
                                   output_view, backend, handle, stream);

            const std::string backend_name = warpforge::gemm_backend_name(backend);
            records.push_back(
                {"miniinfer_" + backend_name + "_kernel.json",
                 make_result(options, fixture, block, backend, "kernel-sequence-device-resident",
                             std::move(kernel_samples), validation.aggregate)});
            records.push_back(
                {"miniinfer_" + backend_name + "_end_to_end.json",
                 make_result(options, fixture, block, backend, "end-to-end-h2d-forward-d2h",
                             std::move(end_to_end_samples), validation.aggregate)});
        }
