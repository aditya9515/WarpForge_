...
- [Hosted CPU workflow](../.github/workflows/cpu.yml): [run 35819598851](https://github.com/aditya9515/WarpForge_/actions/runs/35819598851) passed on Ubuntu 24.04 and Windows Server 2022 for formatted code commit `a077677`. Both jobs compiled and tested the CPU-only mode, checked Python syntax and repository links/JSON, and verified the committed release plot. These jobs make no GPU claim.
- [Formatting policy](../.clang-format) and [check script](../scripts/check_format.py): all 61 C++/CUDA files were formatted with the Ubuntu runner's preinstalled `clang-format-18`, and the same hosted tool now enforces a passing format gate. No local formatter was installed; run `python3 scripts/check_format.py --formatter clang-format-18` on a host where that executable already exists.
